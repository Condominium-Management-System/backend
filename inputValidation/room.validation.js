...
export const roomValidation = Joi.object({
  ...
  condoId: Joi.string()
    ...
    .messages({
      ...
      "string.uuid":
        "Condominium ID must be a valid UUID",
...
export const updateRoomStatusSchema = Joi.object({
  ...
  if (
    ...
    return helpers.message({
      custom:
        "occupiedById is required when room status is occupied"
    });
  }
  ...
  if (
    ...
    value.occupiedById !== undefined
  ) {
    return helpers.message({
      custom:
        "occupiedById must be null when room status is free"
    });
  }
